-- facts/players/on_player_move.lua -- optional OnPlayerMove listener gated by declared interest.
...
if type(moduleName) == "string" then
	...
	if type(loaded) == "table" then
		...
		---@diagnostic disable-next-line: undefined-field
		package.loaded[moduleName] = OnPlayerMove
	end
end
OnPlayerMove._internal = OnPlayerMove._internal or {}

local INTEREST_TYPE_PLAYERS = "players"
local INTEREST_SCOPE_MOVE = "onPlayerMove"
...
local function attachListenerOnce(ctx)
	...
	local fn = function(player)
		...
		local effective = type(effectiveByBucket) == "table" and effectiveByBucket[INTEREST_SCOPE_MOVE] or nil
		...
		local record = players.makePlayerRecord(player, "event", { scope = INTEREST_SCOPE_MOVE })
		if record == nil then
			return
		end

		local moveState = state._playerMove or {}
		state._playerMove = moveState
		local cooldownMs = math.max(0, (tonumber(effective.cooldown) or 0) * 1000)
		emitWithCooldown(moveState, ctx.emitFn, record, nowMillis(), cooldownMs, function()
	...
	handler.Add(fn)
	state.onPlayerMoveHandler = fn
	if not ctx.headless then
		Log:info("OnPlayerMove listener attached")
	end
	return true
end

if OnPlayerMove.ensure == nil then
	--- Ensure the OnPlayerMove listener is attached/detached based on declared interest.
	--- @param ctx table
	--- @return boolean active
	function OnPlayerMove.ensure(ctx)
		ctx = ctx or {}
		local state = ctx.state or {}
		ctx.state = state
		...
		if listenerEnabled then
			effective = InterestEffective.ensure(state, ctx.interestRegistry, ctx.runtime, INTEREST_TYPE_PLAYERS, {
				...
				allowDefault = false,
				log = Log,
				bucketKey = INTEREST_SCOPE_MOVE,
			})
			if effective and ctx.interestRegistry and ctx.interestRegistry.effective then
				local okMerged, merged = pcall(function()
					return ctx.interestRegistry:effective(INTEREST_TYPE_PLAYERS, nil, { bucketKey = INTEREST_SCOPE_MOVE })
				end)
			...
			if type(state._effectiveInterestByType[INTEREST_TYPE_PLAYERS]) == "table" then
				state._effectiveInterestByType[INTEREST_TYPE_PLAYERS][INTEREST_SCOPE_MOVE] = nil
			end
		end
		...
		if wantsListener then
			...
			if not okListener and not ctx.headless then
				Log:warn("OnPlayerMove listener not attached (Events unavailable)")
			end
			return state.onPlayerMoveHandler ~= nil
		end
		...
		if state.onPlayerMoveHandler then
			...
			if handler and type(handler.Remove) == "function" then
				...
				if not ctx.headless then
					Log:info("OnPlayerMove listener detached (no onPlayerMove interest)")
				end
			end
		end
		return false
...
OnPlayerMove._internal.attachListenerOnce = attachListenerOnce
OnPlayerMove._internal.emitWithCooldown = emitWithCooldown
