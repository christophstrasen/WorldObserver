...
describe("WorldObserver runtime scaffold", function()
	...
	describe("controller_tick", function()
		...
		it("degrades when average tick exceeds budget and emits an event", function()
			...
			assert.equals(3, snap.budgets.schedulerMaxItemsPerTick)
		...
		it("degrades on spike above spike budget", function()
			local rt = Runtime.new({
				tickBudgetMs = nil,
				tickSpikeBudgetMs = 8,
				windowTicks = 3,
		...
		it("degrades when ingest backlog rises across windows", function()
			local rt = Runtime.new({
				tickBudgetMs = 100,
				windowTicks = 2,
			})
			-- First window: low pending
			rt:controller_tick({ tickMs = 1, ingestPending = 1, ingestDropped = 0 })
			rt:controller_tick({ tickMs = 1, ingestPending = 1, ingestDropped = 0 })
			assert.equals("normal", rt:status_get().mode)
			-- Second window: higher pending should trigger backlog reason
			rt:controller_tick({ tickMs = 1, ingestPending = 5, ingestDropped = 0 })
			rt:controller_tick({ tickMs = 1, ingestPending = 5, ingestDropped = 0 })
			local snap = rt:status_get()
			assert.equals("degraded", snap.mode)
			assert.equals(Runtime.Reasons.ingestBacklogRising, snap.lastTransitionReason)
		end)
		...
		it("recovers after a clean window and clears degraded budgets", function()
			...
			assert.is_nil(snap.budgets.schedulerMaxItemsPerTick)
		end)
