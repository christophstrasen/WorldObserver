...
| [`players`](players.md)        | `onPlayerMove`, `onPlayerUpdate`                  | n/a                                                | `playerKey` (prefers `steamId`, then `onlineId`, then `playerId`, then `playerNum`) | High-ish (identifier dependent; varies by MP/SP)                    |
