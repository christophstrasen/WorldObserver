...
compute_fingerprint() {
  ...
  # Best-effort fingerprint: file list + size + mtime.
  # (Avoids a hard dependency on a watcher tool.)
...
echo "Watching WorldObserver (TARGET=$TARGET)…"
sync_once
...
if command -v inotifywait >/dev/null; then
  ...
  # Monitor mode avoids missing the "next" event due to watcher re-setup timing.
  # We intentionally ignore the event payload; any change triggers a re-sync.
